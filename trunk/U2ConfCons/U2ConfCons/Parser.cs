    ...
    public class Parser
    {
        ...
        public int[] loadConfig(string filename, int car)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("Неверный *.car Файл");
                }
                if (hdr[4] != 0x01) //версия конфига
                {
                    throw new Exception("Неверная версия *.car файла");
                }
                this.carAddress = Convert.ToInt32("0x" + hdr[5].ToString("X2") + hdr[6].ToString("X2") + hdr[7].ToString("X2") + hdr[8].ToString("X2"), 16);
                ...
                cara = null; //и это как ни странно тоже не нужно
                GC.Collect();
                return conf;
            }
            catch (Exception e)
